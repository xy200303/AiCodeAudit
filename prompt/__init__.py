...
根节点校验信号数量:整数
根节点安全信号数量:整数
上游分支_0:上游输入 -> 中间函数 -> 根节点
下游分支_0:根节点 -> 中间函数 -> 危险调用
重点链路数:整数
<重点链路_0>
...
如果输入中提供了“依赖上下文摘要”和“路径节点详情”，你必须先阅读“依赖上下文摘要”识别根节点周围的上游输入分支、下游危险分支以及重点传播链路，再使用对应节点源码做证据确认。

依赖上下文解读规则：
1. `依赖上下文摘要` 同时描述根节点周围的局部依赖树和重点传播链路，既要看多条上游输入分支是否汇入根节点，也要看根节点是否继续向下游危险调用传播。
2. 根节点通常是最值得审查的汇聚点或转发点，若上游分支出现输入源、下游分支出现危险点，应优先围绕这组上下文判断是否形成真实漏洞链路。
3. `重点链路` 是从依赖树中抽出的高价值传播路径；`贯通调用链` 优先级最高，其次是 `下游危险链` 和 `上游输入链`。
4. 若某条重点链路同时具有较高的“输入源数量”和“危险点数量”，且“校验信号数量”“安全信号数量”偏少，应优先作为风险候选。
5. 若多条分支中只有部分分支带有校验/安全信号，不要默认整棵树安全，要分别确认危险分支是否真正经过了防护逻辑。
6. 若上下文中出现明显的参数化、白名单、规范化路径、鉴权、转义等安全信号，应降低风险判断，并结合源码再次确认。
7. 不要因为树节点多、分支多或链路长就自动判定有风险，仍然必须回到路径节点源码寻找真实证据。
...
4. 若“依赖上下文摘要”中的 `重点链路` 显示存在 `贯通调用链`，且路径节点源码能对应出输入传播和危险落点，应优先沿这条链组织攻击向量描述
