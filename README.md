...
- 新增语义边界切分：LLM 分片会优先在函数 / 类边界断开，减少从函数中间截断导致的理解偏差
- 新增 Tree-sitter 局部语法错误回退：整文件静态解析失败时，会按较大语义块重试，尽量保住可解析区域
- 新增 `Agent_2` 风险候选预筛：优先把带输入源、危险点、校验/安全信号或邻近风险链的节点送入大模型审计
- 新增 `Agent_2` 统一依赖上下文审计：会围绕根节点构建“上游输入分支 + 下游危险分支 + 重点传播链路”的合并上下文，再交给大模型识别风险
- 新增 `Agent_1` 结果二次校验：会过滤自然语言函数名、行号越界、空代码片段或与源码不匹配的异常依赖边
...
项目整体分为两个阶段。第一阶段会扫描项目目录，读取源码与配置文件，并按 token 对源码做切片；对需要进入 LLM 的代码块，系统会优先在函数 / 类等语义边界处断开，尽量避免从函数中间截断。对于 `.py`、`.js`、`.ts`、`.java`、`.go`、`.php`、`.c`、`.cpp`、`.cs` 等语言，系统会优先使用 Tree-sitter 做静态依赖提取；若整文件解析失败但只是局部语法错误，则会按较大语义块重试静态解析，尽量保住可解析区域；若仍无法静态提取，再回退到 Agent_1 进行大模型提取。无论依赖来自静态解析还是大模型，都会经过一层结果二次校验，过滤自然语言函数名、行号越界、空代码片段和与源码不匹配的异常依赖边，最终构建项目调用图。第二阶段会先对图中的节点做风险候选预筛，优先保留带输入源、危险点、校验/安全信号或邻近风险链的高价值节点；随后围绕这些高价值节点构建统一的“依赖上下文摘要”，在同一份上下文里同时保留根节点的上游输入分支、下游危险分支和重点传播链路，再附带关键节点源码，交给 Agent_2 判断是否存在“外部输入 -> 传播节点 -> 危险操作 -> 缺少校验/转义/鉴权”的真实漏洞链路。若统一依赖上下文无法覆盖足够信息，系统仍会回退到节点级局部子图审计。最终输出 `graphml` 依赖图和结构化审计报告，供命令行或 Streamlit 页面展示。
